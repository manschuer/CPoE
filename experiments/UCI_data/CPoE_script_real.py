...
class SCRIPT1:
	def __init__(self, loc_csv, Nreps=1, seed0=0, name='', Ntestmax=1000, NtestFIX=False, FULL=True, kernelMODE=1):
	...
	def runfullGP(self):
		...
		for i in range(self.Nreps):
			...
			pickle.dump( RESs, open( self.name+'_fullGP', 'wb' ) )

			MVs.append( np.vstack([ rFullGP.m, rFullGP.v ]).T )
			pickle.dump( MVs, open( self.name+'_fullGP_MV', 'wb' ) )
	...
	def runCPoE(self, K0, Ps, p, HYPERS='BATCH', TRACE=False, gamma=0.01, E=5, REL=1e-10):
		...
		if self.FULL:
			MVs = pickle.load( open( self.name+'_fullGP_MV', 'rb' ) )
	...
	def runNON_GP(self, ALG='MLP', hidden_layer_sizes=(500,500), nam=''):
		...
		for i in range(self.Nreps):
			...
			pickle.dump( RESs, open( self.name+'_'+ALG+nam, 'wb' ) )
